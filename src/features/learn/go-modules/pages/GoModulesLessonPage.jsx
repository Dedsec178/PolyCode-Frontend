...
const READ_GATE_PREFIX = "go_modules";

export default function GoModulesLessonPage() {
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>Go Modules lesson not found.</p>
  ...
  const earnedXP = GO_MODULES_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="Go Modules"
            syncLabel={
              isAuthenticated
                ? "Go Modules progress saved to your account"
        ...
        <LessonContentShell
          storageKey={`go_modules:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Go Modules`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
