...
import {
  CSHARP_PROJECTS_CHAPTERS,
  CSHARP_PROJECTS_LESSONS,
  CSHARP_PROJECTS_TOTAL_XP,
} from "../data/csharpProjectsCurriculum";
import useCsharpProjectsProgress from "../hooks/useCsharpProjectsProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/csharp-projects";
...
export default function CsharpProjectsHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useCsharpProjectsProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = CSHARP_PROJECTS_LESSONS.filter(
    (lesson) => progress[lesson.id],
  ).reduce((sum, lesson) => sum + lesson.xp, 0);
  const pct =
    Math.round((completedCount / CSHARP_PROJECTS_LESSONS.length) * 100) || 0;
  const nextLesson =
    CSHARP_PROJECTS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    CSHARP_PROJECTS_LESSONS[0];
  const resumeLesson =
    CSHARP_PROJECTS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = CSHARP_PROJECTS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => CSHARP_PROJECTS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return CSHARP_PROJECTS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub csharp-projects-hub">
      <div className="oops-hero csharp-projects-hero">
        <Link
          to="/select-language"
        ...
        <div className="oops-hero-badge">C# · CAPSTONE TRACK</div>
        ...
        <p className="oops-hero-sub">
          Build 4 real, growing projects — a contact book, a persistent
          todo list, a grade analyzer, and a mini task API — applying
          everything from Fundamentals to ASP.NET Basics.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${CSHARP_PROJECTS_LESSONS.length} lessons · ${earnedXP}/${CSHARP_PROJECTS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${CSHARP_PROJECTS_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume C# Projects" : "Start C# Projects"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a C# Projects topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search contacts, todos, grades, APIs..."
              aria-label="Search C# Projects lessons"
            />
            <div className="oops-filter-tabs" aria-label="Filter C# Projects lessons">
              {[
                ["all", "All"],
                ["todo", "To do"],
                ["done", "Done"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{CSHARP_PROJECTS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{CSHARP_PROJECTS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{CSHARP_PROJECTS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <LearnChapterPathOverview
        chapters={CSHARP_PROJECTS_CHAPTERS}
        progress={progress}
      ...
      <CourseCertificate
        ...
        totalLessons={CSHARP_PROJECTS_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={CSHARP_PROJECTS_TOTAL_XP}
      />
    </div>
  );
}
