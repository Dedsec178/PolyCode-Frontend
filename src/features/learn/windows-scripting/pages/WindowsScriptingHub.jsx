...
const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["ws-system-processes"],
    color: "#0ea5e9",
    summary: "Querying system info, listing/filtering running processes, and force-closing them with taskkill.",
  },
  {
    level: "Core Skills",
    chapters: ["ws-registry-config"],
    color: "#6366f1",
    summary: "Reading and writing the registry with reg, and persisting environment variables with setx.",
  },
  {
    level: "Intermediate",
    chapters: ["ws-networking"],
    color: "#f59e0b",
    summary: "Checking network configuration, testing connectivity with ping, and inspecting active connections.",
  },
  {
    level: "Advanced",
    chapters: ["ws-services-users"],
    color: "#dc2626",
    summary: "Managing Windows services and user accounts, then combining everything into a health-check script.",
  },
];
...
export default function WindowsScriptingHub() {
  ...
  const pct =
    Math.round((completedCount / WINDOWS_SCRIPTING_LESSONS.length) * 100) || 0;

  const nextLesson =
    WINDOWS_SCRIPTING_LESSONS.find((lesson) => !progress[lesson.id]) ||
    WINDOWS_SCRIPTING_LESSONS[0];
  const resumeLesson =
    WINDOWS_SCRIPTING_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = WINDOWS_SCRIPTING_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => WINDOWS_SCRIPTING_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return WINDOWS_SCRIPTING_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub matplotlib-hub">
      <div className="oops-hero matplotlib-hero">
        ...
        <div className="oops-hero-badge">BATCHFILE · SCRIPTING COURSE</div>
        <h1 className="oops-hero-title">
          Windows
          <br />
          <span className="oops-hero-accent" style={{ color: "#0ea5e9" }}>
            Scripting
          </span>
        </h1>
        <p className="oops-hero-sub">
          Go beyond core batch syntax into the Windows-specific command-line
          toolkit — system info and process control, the registry, network
          diagnostics, and managing services and user accounts.{" "}
          {WINDOWS_SCRIPTING_CHAPTERS.length} chapters,{" "}
          {WINDOWS_SCRIPTING_LESSONS.length} lessons, hands-on challenges.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${WINDOWS_SCRIPTING_LESSONS.length} lessons · ${earnedXP}/${WINDOWS_SCRIPTING_TOTAL_XP} XP`
                  : `Sign in to track progress · ${WINDOWS_SCRIPTING_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume Windows Scripting" : "Start Windows Scripting"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a Windows Scripting topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search registry, services, networking..."
              aria-label="Search Windows Scripting lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter Windows Scripting lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{WINDOWS_SCRIPTING_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{WINDOWS_SCRIPTING_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{WINDOWS_SCRIPTING_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {WINDOWS_SCRIPTING_CHAPTERS.length} chapters ·{" "}
            {WINDOWS_SCRIPTING_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = WINDOWS_SCRIPTING_CHAPTERS.filter((ch) =>
