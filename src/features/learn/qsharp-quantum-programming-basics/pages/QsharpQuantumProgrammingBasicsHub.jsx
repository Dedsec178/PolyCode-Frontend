...
import {
  QSHARP_QUANTUM_PROGRAMMING_BASICS_CHAPTERS,
  QSHARP_QUANTUM_PROGRAMMING_BASICS_LESSONS,
  QSHARP_QUANTUM_PROGRAMMING_BASICS_TOTAL_XP,
} from "../data/qsharpQuantumProgrammingBasicsCurriculum";
import useQsharpQuantumProgrammingBasicsProgress from "../hooks/useQsharpQuantumProgrammingBasicsProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import LearnChapterIcon from "../../shared/LearnChapterIcon";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/qsharp-quantum-programming-basics";

const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["qpb-superposition"],
    color: "#0284c7",
    summary: "The Hadamard gate (H) and equal superposition states.",
  },
  {
    level: "Intermediate",
    chapters: ["qpb-superposition"],
    color: "#0078d4",
    summary: "State vector preparation and Ry rotation transformations.",
  },
  {
    level: "Advanced",
    chapters: ["qpb-registers"],
    color: "#6366f1",
    summary: "Qubit array allocations (Qubit[n]) and Controlled functors.",
  },
  {
    level: "Pro",
    chapters: ["qpb-registers"],
    color: "#a855f7",
    summary: "Bell state entanglement (|Φ⁺⟩) and multi-qubit correlations.",
  },
  {
    level: "Expert",
    chapters: ["qpb-operators"],
    color: "#ec4899",
    summary: "The Adjoint functor, reversible transformations, and register measurements.",
  },
];
...
export default function QsharpQuantumProgrammingBasicsHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useQsharpQuantumProgrammingBasicsProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = QSHARP_QUANTUM_PROGRAMMING_BASICS_LESSONS.filter(
    (lesson) => progress[lesson.id],
  ).reduce((sum, lesson) => sum + lesson.xp, 0);
  const pct = Math.round((completedCount / QSHARP_QUANTUM_PROGRAMMING_BASICS_LESSONS.length) * 100) || 0;
  const nextLesson =
    QSHARP_QUANTUM_PROGRAMMING_BASICS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    QSHARP_QUANTUM_PROGRAMMING_BASICS_LESSONS[0];
  const resumeLesson =
    QSHARP_QUANTUM_PROGRAMMING_BASICS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = QSHARP_QUANTUM_PROGRAMMING_BASICS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return QSHARP_QUANTUM_PROGRAMMING_BASICS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub matplotlib-hub font-sans pb-24">
      <div className="oops-hero matplotlib-hero">
        ...
        <div className="oops-hero-badge">BASICS TRACK</div>
        <h1 className="oops-hero-title">
          Quantum
          <br />
          <span className="oops-hero-accent">Programming Basics</span>
        </h1>
        <p className="oops-hero-sub">
          Learn quantum superposition, Hadamard transformations, qubit registers, Bell entanglement, and Adjoint operations in Q#.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${QSHARP_QUANTUM_PROGRAMMING_BASICS_LESSONS.length} lessons · ${earnedXP}/${QSHARP_QUANTUM_PROGRAMMING_BASICS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${QSHARP_QUANTUM_PROGRAMMING_BASICS_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume Basics" : "Start Basics"}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="oops-tool-panel oops-tool-panel-main">
          ...
          <div className="oops-search-row">
            <input
              ...
              placeholder="Search Hadamard, superposition, Bell state..."
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>{completedCount}/{QSHARP_QUANTUM_PROGRAMMING_BASICS_LESSONS.length}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>{completedChapters}/{QSHARP_QUANTUM_PROGRAMMING_BASICS_CHAPTERS.length}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>{earnedXP}/{QSHARP_QUANTUM_PROGRAMMING_BASICS_TOTAL_XP}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {QSHARP_QUANTUM_PROGRAMMING_BASICS_CHAPTERS.length} chapters ·{" "}
            {QSHARP_QUANTUM_PROGRAMMING_BASICS_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = QSHARP_QUANTUM_PROGRAMMING_BASICS_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={QSHARP_QUANTUM_PROGRAMMING_BASICS_CHAPTERS}
        progress={progress}
      ...
      {pct >= 100 && (
        <CourseCertificate
          courseName="Quantum Programming Basics"
          totalLessons={QSHARP_QUANTUM_PROGRAMMING_BASICS_LESSONS.length}
          completedCount={completedCount}
          earnedXP={earnedXP}
          totalXP={QSHARP_QUANTUM_PROGRAMMING_BASICS_TOTAL_XP}
        />
      )}
    </div>
  );
