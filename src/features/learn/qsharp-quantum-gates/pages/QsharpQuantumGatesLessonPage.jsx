...
const READ_GATE_PREFIX = "qsharp_quantum_gates";
const ACCENT = "#e11d48";

export default function QsharpQuantumGatesLessonPage() {
  ...
  const lesson = QSHARP_QUANTUM_GATES_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = QSHARP_QUANTUM_GATES_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = QSHARP_QUANTUM_GATES_LESSONS[lessonIdx - 1];
  const next = QSHARP_QUANTUM_GATES_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "Quantum Gates",
    language: "Q#",
    lesson,
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`qsharp_quantum_gates:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Quantum Gates`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
