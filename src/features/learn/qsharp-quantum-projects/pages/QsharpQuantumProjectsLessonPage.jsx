...
const READ_GATE_PREFIX = "qsharp_quantum_projects";
const ACCENT = "#ec4899";

export default function QsharpQuantumProjectsLessonPage() {
  ...
  const lesson = QSHARP_QUANTUM_PROJECTS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = QSHARP_QUANTUM_PROJECTS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = QSHARP_QUANTUM_PROJECTS_LESSONS[lessonIdx - 1];
  const next = QSHARP_QUANTUM_PROJECTS_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "Quantum Projects",
    language: "Q#",
    lesson,
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`qsharp_quantum_projects:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Quantum Projects`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
