...
import {
  QSHARP_QUANTUM_PROJECTS_CHAPTERS,
  QSHARP_QUANTUM_PROJECTS_LESSONS,
  QSHARP_QUANTUM_PROJECTS_TOTAL_XP,
} from "../data/qsharpQuantumProjectsCurriculum";
import useQsharpQuantumProjectsProgress from "../hooks/useQsharpQuantumProjectsProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import LearnChapterIcon from "../../shared/LearnChapterIcon";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/qsharp-quantum-projects";
...
const LEARNING_PATH = [
  {
    ...
    chapters: ["qp-crypto"],
    color: "#ec4899",
    summary: "Quantum Random Number Generator (QRNG) creating hardware-grade true randomness.",
  },
  {
    level: "Intermediate",
    chapters: ["qp-crypto"],
    color: "#d946ef",
    summary: "Quantum Teleportation Protocol using Bell pairs & 2 classical bits.",
  },
  {
    level: "Advanced",
    chapters: ["qp-crypto"],
    color: "#a855f7",
    summary: "BB84 Quantum Key Distribution (QKD) & eavesdropper detection.",
  },
  {
    level: "Pro",
    chapters: ["qp-capstone"],
    color: "#8b5cf6",
    summary: "Superdense coding (transmitting 2 classical bits through 1 physical qubit).",
  },
  {
    level: "Expert",
    chapters: ["qp-capstone"],
    color: "#f43f5e",
    summary: "Variational Quantum Eigensolver (VQE) simulator & full Q# Capstone Project.",
  },
];
...
export default function QsharpQuantumProjectsHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useQsharpQuantumProjectsProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = QSHARP_QUANTUM_PROJECTS_LESSONS.filter(
    (lesson) => progress[lesson.id],
  ).reduce((sum, lesson) => sum + lesson.xp, 0);
  const pct = Math.round((completedCount / QSHARP_QUANTUM_PROJECTS_LESSONS.length) * 100) || 0;
  const nextLesson =
    QSHARP_QUANTUM_PROJECTS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    QSHARP_QUANTUM_PROJECTS_LESSONS[0];
  const resumeLesson =
    QSHARP_QUANTUM_PROJECTS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = QSHARP_QUANTUM_PROJECTS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return QSHARP_QUANTUM_PROJECTS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub matplotlib-hub font-sans pb-24">
      <div className="oops-hero matplotlib-hero">
        ...
        <div className="oops-hero-badge">PROJECTS TRACK</div>
        <h1 className="oops-hero-title">
          Quantum
          <br />
          <span className="oops-hero-accent">Projects</span>
        </h1>
        <p className="oops-hero-sub">
          Build end-to-end Q# quantum applications: Quantum Key Distribution (BB84), Quantum Teleportation, Quantum Random Number Generator, Superdense Coding, and VQE Capstone.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${QSHARP_QUANTUM_PROJECTS_LESSONS.length} lessons · ${earnedXP}/${QSHARP_QUANTUM_PROJECTS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${QSHARP_QUANTUM_PROJECTS_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume Projects" : "Start Projects"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a project</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search QRNG, Teleportation, BB84, VQE..."
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>{completedCount}/{QSHARP_QUANTUM_PROJECTS_LESSONS.length}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>{completedChapters}/{QSHARP_QUANTUM_PROJECTS_CHAPTERS.length}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>{earnedXP}/{QSHARP_QUANTUM_PROJECTS_TOTAL_XP}</strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {QSHARP_QUANTUM_PROJECTS_CHAPTERS.length} chapters ·{" "}
            {QSHARP_QUANTUM_PROJECTS_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = QSHARP_QUANTUM_PROJECTS_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={QSHARP_QUANTUM_PROJECTS_CHAPTERS}
        progress={progress}
      ...
      {pct >= 100 && (
        <CourseCertificate
          courseName="Quantum Projects"
          totalLessons={QSHARP_QUANTUM_PROJECTS_LESSONS.length}
          completedCount={completedCount}
          earnedXP={earnedXP}
          totalXP={QSHARP_QUANTUM_PROJECTS_TOTAL_XP}
        />
      )}
    </div>
  );
