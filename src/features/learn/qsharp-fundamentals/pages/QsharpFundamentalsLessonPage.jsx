...
const READ_GATE_PREFIX = "qsharp_fundamentals";
const ACCENT = "#0078d4";

export default function QsharpFundamentalsLessonPage() {
  ...
  const lesson = QSHARP_FUNDAMENTALS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = QSHARP_FUNDAMENTALS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = QSHARP_FUNDAMENTALS_LESSONS[lessonIdx - 1];
  const next = QSHARP_FUNDAMENTALS_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "Q# Fundamentals",
    language: "Q#",
    lesson,
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`qsharp_fundamentals:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Q# Fundamentals`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
