...
const READ_GATE_PREFIX = "go_concurrency";

export default function GoConcurrencyLessonPage() {
  ...
  const lesson = GO_CONCURRENCY_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = GO_CONCURRENCY_LESSONS.findIndex((item) => item.id === lessonId);
  const prev = GO_CONCURRENCY_LESSONS[lessonIdx - 1];
  const next = GO_CONCURRENCY_LESSONS[lessonIdx + 1];
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>Go Concurrency lesson not found.</p>
  ...
  const earnedXP = GO_CONCURRENCY_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="Go Concurrency"
            syncLabel={
              isAuthenticated
                ? "Go Concurrency progress saved to your account"
        ...
        <LessonContentShell
          storageKey={`go_concurrency:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Go`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
