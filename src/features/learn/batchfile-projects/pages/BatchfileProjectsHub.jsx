...
import {
  BATCHFILE_PROJECTS_CHAPTERS,
  BATCHFILE_PROJECTS_LESSONS,
  BATCHFILE_PROJECTS_TOTAL_XP,
} from "../data/batchfileProjectsCurriculum";
import useBatchfileProjectsProgress from "../hooks/useBatchfileProjectsProgress";
import CourseCertificate from "../../shared/CourseCertificate";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import LearnChapterIcon from "../../shared/LearnChapterIcon";

const BASE_PATH = "/learn/batchfile-projects";
const ACCENT = "#f39c12";

const LEARNING_PATH = [
  {
    level: "Project 1",
    chapters: ["bfp-sysinfo"],
    color: "#f39c12",
    summary: "Filtering systeminfo with findstr and saving a dated report to a file.",
  },
  {
    level: "Project 2",
    chapters: ["bfp-organizer"],
    color: "#0ea5e9",
    summary: "Looping over files by extension and sorting them into folders automatically.",
  },
  {
    level: "Project 3",
    chapters: ["bfp-backup"],
    color: "#22c55e",
    summary: "Timestamped backup folders and mirroring data safely with robocopy.",
  },
  {
    level: "Capstone",
    chapters: ["bfp-toolkit"],
    color: "#dc2626",
    summary: "A looping choice-driven menu that ties the reporter, organizer, and backup tool into one toolkit.",
  },
];
...
export default function BatchfileProjectsHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useBatchfileProjectsProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = BATCHFILE_PROJECTS_LESSONS.filter(
    (lesson) => progress[lesson.id],
  ).reduce((sum, lesson) => sum + lesson.xp, 0);
  const pct =
    Math.round((completedCount / BATCHFILE_PROJECTS_LESSONS.length) * 100) || 0;

  const nextLesson =
    BATCHFILE_PROJECTS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    BATCHFILE_PROJECTS_LESSONS[0];
  const resumeLesson =
    BATCHFILE_PROJECTS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = BATCHFILE_PROJECTS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => BATCHFILE_PROJECTS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return BATCHFILE_PROJECTS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub matplotlib-hub">
      <div className="oops-hero matplotlib-hero">
        ...
        <div className="oops-hero-badge">BATCHFILE · PROJECTS COURSE</div>
        <h1 className="oops-hero-title">
          Batchfile
          <br />
          <span className="oops-hero-accent" style={{ color: "#f39c12" }}>
            Projects
          </span>
        </h1>
        <p className="oops-hero-sub">
          Apply everything from Fundamentals and Automation to four real
          tools — a system info reporter, a file organizer, a backup
          utility, and a capstone menu toolkit that ties them all
          together. {BATCHFILE_PROJECTS_CHAPTERS.length}{" "}
          chapters, {BATCHFILE_PROJECTS_LESSONS.length} lessons, hands-on
          challenges.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${BATCHFILE_PROJECTS_LESSONS.length} lessons · ${earnedXP}/${BATCHFILE_PROJECTS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${BATCHFILE_PROJECTS_LESSONS.length} lessons`}
              </span>
        ...
        <div className="oops-tool-panel oops-tool-panel-main">
          ...
          <div className="oops-search-row">
            <input
              ...
              placeholder="Search report, organizer, backup, menu..."
              aria-label="Search Batchfile Projects lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter Batchfile Projects lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{BATCHFILE_PROJECTS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{BATCHFILE_PROJECTS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{BATCHFILE_PROJECTS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          <span>Your path · Project by project</span>
          <small>
            {BATCHFILE_PROJECTS_CHAPTERS.length} chapters ·{" "}
            {BATCHFILE_PROJECTS_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = BATCHFILE_PROJECTS_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={BATCHFILE_PROJECTS_CHAPTERS}
        progress={progress}
      ...
      <CourseCertificate
        courseName="Batchfile Projects"
        totalLessons={BATCHFILE_PROJECTS_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={BATCHFILE_PROJECTS_TOTAL_XP}
      />
    </div>
  );
}
