...
const READ_GATE_PREFIX = "huggingface_py";
const HUGGINGFACE_ACCENT = "#FF9D00";

export default function HuggingfaceLessonPage() {
  ...
  const lesson = HUGGINGFACE_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = HUGGINGFACE_LESSONS.findIndex((item) => item.id === lessonId);
  const prev = HUGGINGFACE_LESSONS[lessonIdx - 1];
  const next = HUGGINGFACE_LESSONS[lessonIdx + 1];
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>Hugging Face lesson not found.</p>
  ...
  const earnedXP = HUGGINGFACE_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="Hugging Face · py"
            syncLabel={
              isAuthenticated
                ? "Hugging Face progress saved to your account"
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`huggingface-py:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Hugging Face`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
            ...
            <PythonCodeChallenge
              ...
              accentColor={HUGGINGFACE_ACCENT}
